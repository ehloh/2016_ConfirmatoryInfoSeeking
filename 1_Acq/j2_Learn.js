...
// Set up Learning session // ###########################################
{
    ...
    log.afc ={};
    log.afc.qs = [ 'similar to you'];
    log.afc.pair_nrep =2;                       // How many times to repeat each pair in each time we test the 2AFC?
    log.afc.maxblks =log.nblks_infotype;        // How many times will we run the AFC procedure?
    log.afc.nq = log.afc.qs.length;
    log.afc.ntrials = log.afc.qs.length*6*log.afc.pair_nrep*log.afc.maxblks ;
    log.afc.pass_ok_threshold = 1;
    ...
    //if (DeveloperMode) { log.srcstim = ['elephant','butterfly','cat', 'peacock']; pr('FAKE: Specific Src Stims') }
...
}  // Design settings
{
    ...
    var df_q        = new Array(log.afc.ntrials).fill(999),         // 2AFC: Which Q being asked?
        df_srcpair  = new Array(log.afc.ntrials).fill(999),  // 2AFC: Src Pair
        df_src1  = new Array(log.afc.ntrials).fill(999),
        df_src2  = new Array(log.afc.ntrials).fill(999),
        df_cho      = new Array(log.afc.ntrials).fill(999),      // 2AFC: Cho Src
        df_blk      = new Array(log.afc.ntrials).fill(999),    // 2AFC: After which block did this run?
    ...
    w.counterbal_NI_rem = Math.round(Math.random()); // Because not all subjects will do all learning trials (because once they pass they move on), the rounding errors for allocating N & I blocks has to be randomized across subjects (for the overall src agreement to be ok). I've implemented this by saying that we floor/ceil on even/odd blocks, for a given subject. This value here quantifies the remainder (rep num/2)
    ...
    ws.afc_ntrials_pergo =log.afc.ntrials/log.afc.maxblks;      // Every time we trigger an AFC, how many times do we do the trials?
    for (var bl= 0; bl < log.nblks_infotype; bl++ ) {   // For each group of 4 blocks
        wf.bl_tstart = bl*ws.afc_ntrials_pergo;
        wf.bl_tend = (bl+1)*ws.afc_ntrials_pergo-1;
        df_blk = opIndexWriteVec(df_blk, opLinFromTo(wf.bl_tstart, wf.bl_tend, 1), opVecRepmat(ws.afc_blks[bl], log.afc.qs.length*ws.afc_ntrials_pergo));
        wf.afc_qs =  shuffle(opLinFromTo(1, log.afc.qs.length, 1));
        //pr('BLK ' + bl + ' --------------' )

        for (var q = 0; q < log.afc.qs.length; q++ ) {   // For each Q
            wf.q_tstart = wf.bl_tstart +  (q*(log.afc.pair_nrep*6));
            wf.q_tend = wf.bl_tstart +  (q+1)*(log.afc.pair_nrep*6) -1;
            df_q = opIndexWriteVec(df_q, opLinFromTo(wf.q_tstart, wf.q_tend, 1), opVecRepmat(wf.afc_qs[q], log.afc.pair_nrep*6));
            df_srcpair = opIndexWriteVec(df_srcpair, opLinFromTo(wf.q_tstart, wf.q_tend, 1), shuffle(opVecRepmat(opLinFromTo(1,6,1),log.afc.pair_nrep)));
    ...
    //pr('Check trials: ' + dc_trial);

    for (var i= 0; i< dc_trial.length; i++ ) {  // Catch questions should not be the last or first of each block
         if (d_source[dc_trial[i]] != d_source[dc_trial[i]+1]) {dc_trial[i]--;}
         else if (dc_trial[i]>1 & d_source[dc_trial[i]] != d_source[dc_trial[i]-1]) {dc_trial[i]++;}
     }
...
function pres2afc(){
    ...
    // Set up 2AFC trial
    df_src1[fn]= log.srcpair_src[df_srcpair[fn]-1][0];
    df_src2[fn]= log.srcpair_src[df_srcpair[fn]-1][1];
    if (Math.round(Math.random())) {  // Flip L/R presentation
        df_src1[fn] = df_src2[fn];
        df_src2[fn] = log.srcpair_src[df_srcpair[fn]-1][0];
    }
    wf.respOK = 0;
    $('.ChooseInfo1').html('<img src="Stim/src_' + log.srcstim[ df_src1[fn]-1] + '.png" height="' + String(log.SrcSize) + 'px" width="' + String(log.SrcSize) + 'px" alt =' + log.srcstim[df_src1[fn]-1] + ' >')
        .css({ 'position': 'fixed', 'top': '50%', 'left': '20%', 'transform': 'translate(-50%, -50%)'}).show();
    $('.ChooseInfo2').html('<img src="Stim/src_' + log.srcstim[df_src2[fn]-1] + '.png" height="' + String(log.SrcSize) + 'px" width="' + String(log.SrcSize) + 'px" alt =' + log.srcstim[df_src2[fn]-1] + ' >')
        .css({ 'position': 'fixed', 'top': '50%', 'left': '80%', 'transform': 'translate(-50%, -50%)'}).show();
    if (fn==0) { wf.col = 0 }  else if  (df_q[fn] != df_q[fn-1]) { wf.col++; }  // Sprite colours
    if (log.afc.qs[df_q[fn]-1].indexOf('similar')== -1) { // If this isn't a question about similarity
        $(".ChooseInfoText").html("<br><b>Which player/source do you think is more " + log.afc.qs[df_q[fn]-1] + "?</b> </br></br>" +
                "Use the left and right arrows to respond<br><br><br>")
            .css({'background': qcolors[wf.col],  'top': '10%', 'font-size': '0.8em', 'font-weight': 'normal'}).show();
    }
    else {
        $(".ChooseInfoText").html("<b><br>Which player do you think is more " + log.afc.qs[df_q[fn]-1] + "?</b> </br></br>" +
    ...
    if (DeveloperMsg==1) {
        $(".DevMsgStart").html("[AFC trial " + fn.toString() + "] <br><br>" +
            "Sources: " + log.srcstim[df_src1[fn]-1] + '    vs      ' + log.srcstim[ df_src2[fn]-1] + '   (' + log.Src.condition[df_src1[fn]-1] + ' vs ' + log.Src.condition[df_src2[fn]-1] + ') <br>' +
            "Question: " +  log.afc.qs[df_q[fn]-1] + "<br><br>" +
    ...
    if (DeveloperAutoChoice == 1) {
        ...
        // Always choose correct answer
        wf.srcpair_correct=[1,1,1,3,4,0];   // Correct answer: which is more similar, for each source pair?
        //wf.blk_pcorr=[0.5,0.65,0.8, 0.95];  // pCorrect, per AFC block
        wf.blk_pcorr=[0.7,0.8, 0.9, 0.95];  // pCorrect, per AFC block
        //wf.blk_pcorr=[0,0,0,0];  // pCorrect, per AFC block
        //wf.blk_pcorr=[1,1,1,1];  // pCorrect, per AFC block

        df_cho[fn] = wf.srcpair_correct[df_srcpair[fn]-1];
        pr('AFC tn' + fn + ': change to rand, p= ' + wf.blk_pcorr[(df_blk[fn]/4)-1])
        if (Math.random() > ( wf.blk_pcorr[(df_blk[fn]/4)-1])) {    // Probabilistically change to random (as a function of progression of learning blocks)
            df_cho[fn] = opFmat([df_src1[fn], df_src2[fn]], Math.round(Math.random()));
        }

        ////// Random choice
        //df_cho[fn] = log.srcpair_src[df_srcpair[fn] - 1][Math.round(Math.random())];

        setTimeout(function () {
            // COPIED FROM BELOW ################################
            if (wf.respOK > 0.5) {  // If response is OK
                ...
                // Message
                wf.endmsg = "[AFC trial " + fn.toString() + "] <br><br>" +
                    "Sources: " + log.srcstim[df_src1[fn] - 1] + '    vs      ' + log.srcstim[df_src2[fn] - 1] + '   (' + log.Src.condition[df_src1[fn] - 1] + ' vs ' + log.Src.condition[df_src2[fn] - 1] + ') <br>' +
                    "Question: " + log.afc.qs[df_q[fn] - 1] + "<br><br>" +
                    "Chose:   " + log.srcstim[df_cho[fn] - 1] + "    (" + opFmat(['Left', 'Right'], wf.respOK - 1) + ")";

                // Continue with trials
                setTimeout(function () {
                    if (fn == df_blk.length - 1) {           // END of last AFC block
                        if (DeveloperMsg == 1) { $(".DevMsgEnd").html("<b>LAST AFC BLOCK REACHED WITHOUT PASSING. Moving on</b><br>" + wf.endmsg) }
                        $('.wrap_LearnTrial').hide();
                        $('.wrap_Learn2AFC').hide();
                        opPostData();
                    }
                    else if (df_blk[fn] != df_blk[fn + 1]) {   // End of this AFC block
                        wf.blktrials =  opNumFindInd(df_blk,df_blk[fn]);    // Trials to evaluate
                        wf.blkacc = new Array(wf.blktrials.length).fill(999);

                        // Score accuracy for each pair
                        for (var i= 0; i < wf.blktrials.length; i++ ) {
                            switch (df_srcpair[wf.blktrials[i]]) {
                                case 1: // Pair 1: C vs D ==> C (1)
                                    wf.blkacc[i] =  +(df_cho[wf.blktrials[i]] == 1);
                                    break;
                                case 2: // Pair 2: C vs N ==> C (1)
                                    wf.blkacc[i] =  +(df_cho[wf.blktrials[i]] == 1);
                                    break;
                                case 3: // Pair 3: C vs I ==> C (1)
                                    wf.blkacc[i] =  +(df_cho[wf.blktrials[i]] == 1);
                                    break;
                                case 4: // Pair 4: D vs N ==> N (3)
                                    wf.blkacc[i] =  +(df_cho[wf.blktrials[i]] == 3);
                                    break;
                                case 5: // Pair 5: D vs I ==> I (4)
                                    wf.blkacc[i] =  +(df_cho[wf.blktrials[i]] == 4);
                                    break;
                                case 6: // Pair 5: D vs I ==> Fake correct (doesn't really apply)
                                    wf.blkacc[i] =  1;
                                    break;
                            }
                        }

                        // Has subject passed thresholding?
                        if (opMean(wf.blkacc)==log.afc.pass_ok_threshold) {
                            log.afc.pass_ok = 1;
                            log.afc.pass_ok_blk =  df_blk[fn];

                            if (DeveloperMsg == 1) { $(".DevMsgEnd").html("<b>AFC THRESHOLDING PASSED. GO TO NEXT STAGE</b><br>" + wf.endmsg) }
                            ...
                            if (DeveloperMsg == 1) { $(".DevMsgEnd").html("<b>AFC BLOCK SCORE: " + opMean(wf.blkacc) + ". CONTINUE W LEARNING TRIALS</b><br>" + wf.endmsg) }
                            pr("                 == > AFC BLOCK SCORE: " + opMean(wf.blkacc))
                            fn++;
                            tn++;
        ...
        $(document).on('keydown.askafc', function (event) {
            wf.askafc_keydown = new Date();
            switch (event.keyCode) {
                case log.key.left:
                    df_cho[fn] = df_src1[fn];
                    wf.respOK = 1;
                    break;
                case log.key.right:
                    df_cho[fn] = df_src2[fn];
                    wf.respOK = 2;
                    break;
            }
            if (wf.respOK > 0.5) {  // If response is OK
...
function opEndTrial(){
    ...
    if (tn>1 && d_block[tn+1]!=d_block[tn]) {  // Next-block / End of expt (via 2AFC)
    ...
    else if (tn+1== dc_trial[cn])  {  // Catch questions
        presCatchQ()
    }
    else if (tn==log.ntrials) { // If something bad happens
        pr("WARNING: Stopped task because n trials was at max, but via stopgap. This shouldn't be happening.");
        opPostData();
    }
...
function opPostData(){
    ...
    var postdata= markname + "subject" + markeq + log.job.worker_id + marknext
        ...
        + markname + "df_acqsrcpair" + markeq + df_srcpair + marknext
        + markname + "df_cho" + markeq + df_cho + marknext
        + markname + "df_blk" + markeq + df_blk + marknext
        + markname + "df_src1" + markeq + df_src1 + marknext
        + markname + "df_src2" + markeq + df_src2 + marknext
    ...
    if (DeveloperMsg) {$('.DevMsgStart').html('Done<br><br>' +  postdata ).show() }
}
function presBreakScreen() {
    ...
    $('.StartEnd_text').html("<br><br><br>Well done! Next, you will learn about the player who is represented by the " + wt.pic_name.toUpperCase() + "<br><br><br><br>"
            ...
            + "You have finished " + String(tn-1) + " out of a maximum of " + String(d_choice.length) + " trials in this session"
            + "<br><br><br><br>Press any key to continue with the experiment")
        .css({ 'position': 'fixed', 'top': '80%', 'left': '50%', 'transform': 'translate(-50%, -50%)'})
        .show();

    // Fake: Waiting for other players to catch up
    wt.fakewait =  +(Math.random()<0.25)*(5+ Math.random()*6)*1000*(1-DeveloperSpeedUp);
...
function presChooseAvatar() {
    ...
    if (DeveloperAutoChoice == 1) {
        ...
        // Continue w trials - COPIED FROM BELOW
